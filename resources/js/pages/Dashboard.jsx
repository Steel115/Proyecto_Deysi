        ...
        >
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="mb-4">
                        <form onSubmit={handleSearch}>
                            <input
                                ...
                                focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-8">
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 border ${!filters.category
                        ...
                        {categories.map((category) => (
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 border ${filters.category == category.id
                                    ? 'bg-indigo-600 text-white border-transparent shadow'
                                    : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700'
                                    }`}
                            >
                            ...
                            products.data.map((product) => (
                                <div key={product.id} className="bg-white overflow-hidden shadow-lg rounded-xl transition duration-300 transform hover:scale-[1.02] hover:shadow-2xl border border-gray-200 flex flex-col dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        {product.category && (
                                            <span
                                                className={`mb-2 self-start text-xs font-semibold px-2.5 py-0.5 
                                                    rounded-full ${getCategoryClasses(product.category.name)}`}>
                                        ...
                                        <div className="text-sm border-t pt-3 mt-auto space-y-2 dark:border-gray-700">
                                            <p className="font-semibold text-gray-700 dark:text-gray-300">Stock: 
                                                <span className={`text-xs px-2 py-1 leading-5 rounded-full ${product.stock > 10 ?
                                        ...
                                        <div className="mt-6"><button onClick={() => handleDetail(product)} className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-lg cursor-pointer">Ver Detalle</button></div>
                                    </div>
                                </div>
                            ))
                        )}
            ...
            {selectedProduct && (
                <div className="fixed inset-0 bg-black/70 dark:bg-black/80 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-2xl shadow-2xl 
                    max-w-lg w-full p-8 transition-all transform scale-100 border border-gray-200 dark:border-gray-700">

                        {/* Encabezado */}
                        <h2 className="text-3xl font-bold text-gray-800 dark:text-white border-b 
                        border-gray-200 dark:border-gray-700 pb-3 mb-4 flex items-center justify-between">
                            Detalle del Producto
                            <button
                                onClick={closeModal}
                                className="text-gray-500 hover:text-gray-800 dark:text-gray-400 
                                dark:hover:text-gray-200 transition duration-150"
                            >
                                <svg
                        ...
                        <div className="space-y-4">
                            ...
                            {selectedProduct.category && (
                                <p className="text-sm text-gray-600 dark:text-gray-400 italic flex items-center">
                                    Categoría:{" "}
                                    <span
                                        className={`ml-1 px-2 py-0.5 rounded-full text-xs font-semibold ${getCategoryClasses(selectedProduct.category.name)}`}
                                    >
                        ...
                        <div className="mt-6 flex justify-end space-x-4">
                            <button
                                ...
                                className="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 
                                           dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 font-bold 
                                           py-2 px-4 rounded-lg transition duration-150 shadow-md focus:outline-none 
                                           focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600 cursor-pointer"
                            >
                                Cerrar
                            </button>
                            <button
                                ...
                                rounded-lg transition duration-150 shadow-md focus:outline-none focus:ring-4 
                                focus:ring-green-400 cursor-pointer"
                            >
                                Añadir al Carrito
                            </button>
                        </div>
